 ...
 *
 * Uses `--use-api` so Docker is not required (laptop Docker is often down;
 * GitHub runners also have no local Docker daemon for the CLI bundler).
 *
 * Does NOT:
...
export const MISSING_TOKEN_ERROR =
  'SUPABASE_ACCESS_TOKEN is missing. Refusing to deploy admin-action. Set a personal access token locally (supabase login / dashboard Access Tokens). Docker is not required; this command uses --use-api.';
...
function main(argv = process.argv, env = process.env) {
  ...
  if (!execute) {
    console.log('Would deploy ONLY admin-action (get_site_content) via --use-api — homepage untouched:');
    console.log(`  ${printed}`);
    console.log('Not deployed. Pass --execute to run that command.');
    return 0;
  }

  console.log(`Deploying ONLY ${ADMIN_ACTION_FUNCTION} → ${LIVE_SUPABASE_PROJECT_REF} (--use-api, no Docker)`);
  console.log(`$ ${printed}`);
