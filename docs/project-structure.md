...
- **Environment:** `.env.local` (gitignored) for `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` / `VITE_TURNSTILE_SITEKEY`. Owner password is `ADMIN_OWNER_PASSWORD` on Supabase Edge only — never a `VITE_` var. Never commit real keys. See README.md Security section.
