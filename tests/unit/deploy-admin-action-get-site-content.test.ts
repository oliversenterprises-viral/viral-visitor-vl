...
describe('deploy admin-action only (get_site_content) — not executed', () => {
  const src = readFileSync(SCRIPT, 'utf8');
  const pkg = readFileSync(resolve(ROOT, 'package.json'), 'utf8');

  it('targets live project and only admin-action', () => {
    ...
    expect(formatDeployCommand(cmd)).toBe(
      'npx supabase functions deploy admin-action --project-ref wqbefjzpgsezzwdrvvua --yes',
    );
