...
describe('owner funnel desk metrics', () => {
  ...
  it('honest-empties six tiles when the server misses after login, not fake zeros', () => {
    const el = document.createElement('div');
    renderOwnerFunnelDeskView(
      el,
      {
  ...
  it('paints the live snake_case get_owner_funnel_desk envelope, including extra junk_visits', () => {
    const live = {
      window_days: 7,
      visits: 3,
      junk_visits: 530,
  ...
  it('marks a timed-out get_owner_funnel_desk fetch as honest-empty, not hung zeros', () => {
    const loaded = ownerFunnelDeskFromInvokeResult({
      success: false,
    ...
    expect(el.textContent).not.toMatch(/can.t load/i);
    const tiles = el.querySelector('[data-owner-desk-tiles]')?.textContent || '';
    expect(tiles).not.toMatch(/\b0%\b/);
  });

  it('does not paint hung zeros when last-N deskStatus is timeout', () => {
    const loaded = ownerFunnelDeskFromInvokeResult({
      success: true,
      data: {
        windowDays: 7,
  ...
  it('paints honest zeros when last-N queries finished empty', () => {
    expect(deskStatusForPaint({ hasSignal: true, timedOut: true })).toBe('ok');
    expect(deskStatusForPaint({ hasSignal: false, timedOut: true })).toBe('timeout');
    expect(deskStatusForPaint({ hasSignal: false, timedOut: false })).toBe('empty');
    const loaded = ownerFunnelDeskFromInvokeResult({
      success: true,
      data: {
        windowDays: 7,
  ...
  it('refresh keeps the desk instead of toasting can’t load', () => {
    ...
    expect(src).toContain("renderOwnerFunnelDeskView(container, EMPTY_METRICS, 'honest-empty')");
    expect(src).not.toMatch(/showToast\('can.t load/);
  });

  it('Command desk fetches get_owner_funnel_desk with AbortController ≤8s and last-N, GSC untouched', () => {
    const src = readFileSync(resolve(import.meta.dirname, '../../src/admin/owner-funnel-desk.ts'), 'utf8');
    ...
    const gsc = readFileSync(
      resolve(import.meta.dirname, '../../supabase/functions/_shared/owner-funnel-gsc.ts'),
      'utf8',
    );
    const html = readFileSync(resolve(import.meta.dirname, '../../index.html'), 'utf8');
    const client = readFileSync(resolve(import.meta.dirname, '../../src/lib/admin-action-client.ts'), 'utf8');
    ...
    expect(gsc).toContain("GSC_SERVICE_ACCOUNT_JSON");
    expect(gsc).toContain('GSC_RESOLVE_TIMEOUT_MS');
