...
describe('admin-action-client', () => {
  ...
  it('timed invokeAdminAction aborts from an outer signal without functions.invoke', async () => {
    vi.stubEnv('VITE_SUPABASE_URL', 'https://example.supabase.co');
    vi.stubEnv('VITE_SUPABASE_ANON_KEY', 'anon-key');
    vi.resetModules();
    setAdminSessionToken('session-token-abc');
    ...
    const pending = invokeAdminAction('get_owner_funnel_desk', {}, { signal: ctrl.signal });
    ctrl.abort();
    const result = await pending;
    expect(result.success).toBe(false);
    if (!result.success) expect(result.error).toMatch(/timed out|abort/i);
    expect(invoke).not.toHaveBeenCalled();
  });
