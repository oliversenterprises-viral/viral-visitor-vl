...
describe('owner funnel GSC tracker', () => {
  ...
  it('wires get_owner_funnel_desk to attach metrics.gsc', () => {
    ...
    expect(src).toMatch(/DESK_COUNTS_TIMEOUT_MS/);
    expect(src).toMatch(/pageMetricsWindow/);
    expect(src).toMatch(/OWNER_FUNNEL_WINDOW_LIMIT/);
    expect(src).toMatch(/DESK_WINDOW_TIMEOUT_MS/);
    expect(src).toMatch(/isEmptyOwnerFunnelDeskCounts/);
    expect(src).toMatch(/loadCompleteWindow: loadedWindow/);
    expect(src).not.toMatch(/loadCompleteWindow: async \(\) => emptyFeed/);
    expect(src).toMatch(/Promise\.all\(\[\s*withTimeout\(countsPromise/);
