...
try {
  ...
  // Reddit pixel is optional (VITE_REDDIT_PIXEL_ID). Report only — never fail the audit.
  pass(
    'Reddit pixel (optional)',
    checks.pixelScript ? 'pixel.js present (retargeting enabled)' : 'not loaded (env empty or not built-in)',
  );
