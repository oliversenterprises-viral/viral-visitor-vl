...
function renderActivityRow(row: PublicActivityRow, index: number): string {
  ...
  const reveal = ` vr-reveal-row" style="--vr-stagger:${index}`;
...
export function buildRecentActivityHtml(rows: readonly PublicActivityRow[]): string {
  if (!rows.length) {
    return `<div class="text-center py-4 text-zinc-400 text-sm public-empty-state">
      <p class="font-medium text-zinc-300 mb-1">Activity feed is live</p>
      <p>Referrals and shares from real participants will appear here in real time.</p>
    </div>`;
  }
