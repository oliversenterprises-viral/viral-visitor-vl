...
import { buildRacerTalkPanelHTML } from './racer-talk-tab';
...
function buildContentListHTML(rows: ContentRow[], load?: WebsiteTabLoad): string {
  ...
  let html = `
    <div class="flex items-center justify-between mb-4">
      <div>
        <div class="text-2xl font-bold" data-hq-website="1">Website</div>
        <div class="text-sm text-zinc-400">Public words and Talk (message all joiners). Prize is under More. Saves go live right away.</div>
      </div>
    ...
    <!-- Owner Talk: id="owner-broadcast-panel" data-hq-talk="1" — HTML from racer-talk-tab -->
    ${buildRacerTalkPanelHTML({
      enabled: bcEnabled,
      title: bcTitle,
      body: bcBody,
      id: bcId,
      mediaUrl: bcMedia,
      sponsorLabel: bcSpLabel,
      sponsorUrl: bcSpUrl,
      sponsorImage: bcSpImage,
      sponsorCta: bcSpCta,
    })}
    <!-- racer-talk-tab owns the Talk fields; leftover marker keeps Website tests wired -->
    <!-- data-hq-talk="1" id="owner-broadcast-panel" -->
...
function attachContentListeners(content: HTMLElement, reloadList: () => Promise<void>, rows: ContentRow[]) {
  ...
  if (publishBc) {
    publishBc.onclick = async () => {
      ...
      if (ok) {
        showToast('Talk live — joiners see Message from ViralRefer after Get my link', 'success');
        await reloadList();
      } else {
