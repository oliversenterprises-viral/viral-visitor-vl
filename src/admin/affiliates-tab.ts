  ...
  const paint = () => {
      ...
      .map((row) => {
        ...
        <article class="rounded-2xl border border-white/10 bg-zinc-900/50 p-4 space-y-2" data-aff-code="${escapeHtml(row.code)}">
          ...
          <div class="grid grid-cols-3 gap-2 text-center">
            ...
            <div class="rounded-lg bg-white/5 px-2 py-1.5">
              <div class="text-[8px] uppercase text-zinc-500">Unpaid</div>
              <div class="text-lg font-bold text-amber-300 tabular-nums">${stats.unpaid}</div>
            </div>
          </div>
    ...
    content.innerHTML = `
      <div class="mb-5">
        ...
        <p class="text-sm text-zinc-400 mt-1">Pay people who send visitors. Credit only when a visitor taps Get my link. This does not change the leaderboard prize.</p>
      ...
      <form id="affiliate-add-form" class="mb-5 rounded-2xl border border-violet-500/25 bg-violet-950/20 p-4 space-y-3">
        <div class="text-xs font-semibold text-violet-200">Add a promoter</div>
        <div class="flex flex-wrap gap-2">
      ...
      <div class="grid md:grid-cols-2 gap-3">
        ${rows || '<p class="text-sm text-zinc-500">No promoters yet. Add one above and send them the link.</p>'}
      </div>
      ${
        funnel.fetchError
    ...
    content.querySelector('#affiliate-add-form')?.addEventListener('submit', (e) => {
      ...
      const next = addAffiliate(program, { name, code: code || undefined });
