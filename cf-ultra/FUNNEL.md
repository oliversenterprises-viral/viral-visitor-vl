...
TE / rotators are a **distribution path**, not a funnel step. Iframe compact units (`/te?src=te&camp=…&size=468x60`), not `/`. Send human clicks to `/splash?src=te&camp=…` (homepage-like conversion page). Compact impressions do not write KV. Full splash beacons `te_splash_view` / `te_splash_cta` only — never a referral credit. `src=te` Get-my-link does not count as Credit. Owner HQ rolls **Src** (`src=` / empty → `unknown`) the same way as `camp=` for the selected window. See README “Traffic exchanges”.
